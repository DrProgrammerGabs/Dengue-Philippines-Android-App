...
/* renamed from: com.hl.android.view.compositeview.AdapterViewPageVer */
public class AdapterViewPageVer extends AdapterView<Adapter> implements OnGestureListener, OnScaleGestureListener, Runnable {
    ...
    public void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (!this.mResetLayout) {
            if (cv != null) {
                ...
                if (cv.getTop() + cv.getMeasuredHeight() + cvOffset.y + 10 + this.mYScroll < getHeight() / 2 && this.mCurrent + 1 < this.mAdapter.getCount()) {
                    postUnsettle(cv);
                    post(this);
                    this.mCurrent++;
                    onMoveToChild(this.mCurrent);
                }
                if (((cv.getTop() - cvOffset.y) - 10) + this.mYScroll >= getHeight() / 2 && this.mCurrent > 0) {
                    postUnsettle(cv);
                    post(this);
                    this.mCurrent--;
                    onMoveToChild(this.mCurrent);
        ...
        if (this.mCurrent > 0) {
            ...
            int gap = subScreenSizeOffset(lv).y + 20 + cvOffset2.y;
            lv.layout(((cvLeft + cvRight) - lv.getMeasuredWidth()) / 2, (cvTop - gap) - lv.getMeasuredHeight(), ((cvLeft + cvRight) + lv.getMeasuredWidth()) / 2, cvTop - gap);
        }
        if (this.mCurrent + 1 < this.mAdapter.getCount()) {
            View rv = getOrCreateChild(this.mCurrent + 1);
            int gap2 = cvOffset2.y + 20 + subScreenSizeOffset(rv).y;
            rv.layout(((cvLeft + cvRight) - rv.getMeasuredWidth()) / 2, cvBottom + gap2, ((cvLeft + cvRight) + rv.getMeasuredWidth()) / 2, rv.getMeasuredHeight() + cvBottom + gap2);
        }
        invalidate();
    }
    ...
    private void postSettle(final View v) {
        post(new Runnable() {
            public void run() {
                AdapterViewPageVer.this.onSettle(v);
            }
        });
    }
    ...
    private void postUnsettle(final View v) {
        post(new Runnable() {
            public void run() {
                AdapterViewPageVer.this.onUnsettle(v);
            }
        });
    }
