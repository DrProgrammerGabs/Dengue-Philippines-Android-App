...
import com.p000hl.android.view.widget.ImageCacheWidget;
import com.p000hl.android.view.widget.image.CommonImageView;
import java.io.InputStream;
import java.util.ArrayList;

/* renamed from: com.hl.android.view.component.moudle.HLViewFlipper */
public class HLViewFlipper extends FrameLayout implements Component, ComponentPost {
    ...
    private final Handler mHandler = new Handler() {
        public void handleMessage(Message msg) {
            if (msg.what == 1 && HLViewFlipper.this.mRunning) {
                HLViewFlipper.this.showNext();
                sendMessageDelayed(obtainMessage(1), (long) HLViewFlipper.this.mFlipInterval);
            }
        }
    };
    ...
    private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
        public void onReceive(Context context, Intent intent) {
            ...
            if ("android.intent.action.SCREEN_OFF".equals(action)) {
                HLViewFlipper.this.mUserPresent = false;
                HLViewFlipper.this.updateRunning();
            } else if ("android.intent.action.USER_PRESENT".equals(action)) {
                HLViewFlipper.this.mUserPresent = true;
                HLViewFlipper.this.updateRunning();
            }
        }
    };
    ...
    public HLViewFlipper(Context context, ComponentEntity entity2) {
        super(context);
        this.entity = entity2;
    ...
    private void loadBitmap() {
        ...
        ArrayList<CommonImageView> imageViewList = new ArrayList<>();
        if (sourceIDS != null && sourceIDS.size() > 0) {
            for (int i = 0; i < sourceIDS.size(); i++) {
                CommonImageView imageView = new CommonImageView(getContext());
                imageView.measure(MeasureSpec.makeMeasureSpec(lp.width, 1073741824), MeasureSpec.makeMeasureSpec(lp.height, 1073741824));
                addView(imageView);
                imageViewList.add(imageView);
            }
        }
        this.imageCacheWidget = new ImageCacheWidget(getContext(), sourceIDS, imageViewList);
    }
    ...
    public void showOnly(int childIndex) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (i == childIndex) {
                this.imageCacheWidget.loadBitmap(childIndex);
